...
impl FlashBuffer {
    ...
    pub unsafe fn new() -> Self {
        ...
        info!(
            "Flash buffer initialized at address: 0x{:08X}",
            buffer_ptr as u32
        );
    ...
    fn read_magic(&self) -> u32 {
        unsafe { ptr::read_volatile(self.buffer_ptr as *const u32) }
    }

    /// Read the target flash address
    pub fn read_address(&self) -> u32 {
        unsafe { ptr::read_volatile(self.buffer_ptr.add(4) as *const u32) }
    }

    /// Read the data length
    pub fn read_length(&self) -> u32 {
        unsafe { ptr::read_volatile(self.buffer_ptr.add(8) as *const u32) }
    }

    /// Read the current status
    pub fn read_status(&self) -> BufferStatus {
        let status_value = unsafe { ptr::read_volatile(self.buffer_ptr.add(12) as *const u32) };
    ...
    pub fn clear(&self) {
        unsafe {
            ...
            // Set status to Idle
            ptr::write_volatile(
                self.buffer_ptr.add(12) as *mut u32,
                BufferStatus::Idle as u32,
            );
        }
    }
