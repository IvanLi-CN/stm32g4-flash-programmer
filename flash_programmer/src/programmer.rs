    ...
    SPI::Error: core::fmt::Debug,
{
    ...
    pub async fn erase_sector(&mut self, sector: u32) -> Result<(), Error<SPI::Error, DummyError>> {
        ...
        info!(
            "Erasing sector {} at address 0x{:06X}",
            sector,
            sector * w25::Q::SECTOR_SIZE
        );
    ...
    pub async fn program_data(
        ...
        info!(
            "Programming {} bytes to Flash at address 0x{:06X}",
            data.len(),
            address
        );
        ...
        info!(
            "After sector erase, data at 0x{:06X}: {:?}",
            address, erase_verify
        );

        // Write data
        info!("Writing data...");
        info!(
            "About to write {} bytes starting with: {:?}",
            data.len(),
            &data[..data.len().min(8)]
        );
        ...
        self.flash
            .read(address, &mut immediate_verify[..verify_size])
            .await?;
        info!(
            "Immediate readback (first {} bytes): {:?}",
            verify_size,
            &immediate_verify[..verify_size]
        );
        Ok(())
    }
    ...
    #[allow(dead_code)]
    pub async fn program_and_verify(
        &mut self,
        address: u32,
        data: &[u8],
    ) -> Result<(), Error<SPI::Error, DummyError>> {
    ...
    pub async fn dump_flash(
        ...
        info!(
            "Flash dump from 0x{:06X} to 0x{:06X}:",
        ...
        while current_address < end_address {
            ...
            self.flash
                .read(current_address, &mut buffer[..chunk_size])
                .await?;
